...
- [ ] T003 [TDD-RED] Add run ID generation and propagation tests in `ares/tests/observability.rs`
- [ ] T004 [TDD-GREEN] Define typed `RunId` in `ares/src/observability/mod.rs`
- [ ] T005 [TDD-GREEN] [US1] Add CLI option to accept run ID in `ares/src/cli.rs`
...
- [ ] T007 [TDD-RED] Add log redaction tests in `ares/tests/observability.rs`
- [ ] T008 [TDD-RED] Add structured event tests for command, attack, evaluator, timeout, replay, and report stages in `ares/tests/observability.rs`
- [ ] T009 [TDD-GREEN] Implement `tracing-subscriber` initialization and verbosity settings in `ares/src/observability/mod.rs`
- [ ] T010 [TDD-GREEN] Implement redaction policy in `ares/src/observability/redaction.rs`
- [ ] T011 [TDD-GREEN] [US2] Add structured tracing events across CLI, runner, evaluator, adapter, and reporting modules
- [ ] T012 [TDD-GREEN] Ensure logs include required metadata fields without raw prompts/responses by default and point to controlled evidence references when retained
