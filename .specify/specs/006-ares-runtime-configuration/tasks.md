...
- 2026-05-28: Completed fixture/report path coverage and `run --config` usage. Red phase: `cargo test -p ares --test config_loading cli_overrides_take_precedence_for_fixture_and_report_paths` failed because `RuntimeLimits` and `CliConfigOverrides` lacked fixture/report fields. Green phase added `attack_fixture`, `report_path`, ENDI command path overrides, and max concurrency overrides to typed config plus `ares/config/ares.example.toml`. Red phase: `cargo test -p ares --test cli_run cli_run_uses_config_file_fixture_and_report_defaults` failed because `ares run --config` did not load configured paths. Green phase wired config loading into `ares run`, with direct flags overriding config values. Final validation passed: `cargo fmt --all --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo test --workspace --all-features`.
